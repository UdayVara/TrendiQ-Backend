...
export class EnviromentService {
  ...
  async create(createEnviromentDto: CreateEnviromentDto, userId: string) {
    try {
      ...
      if (newEnviroment) {
        return { statusCode: 201, message: 'Enviroment Updated Successfully',enviroment:newEnviroment };
      } else {
  ...
  async update(updateEnviromentDto: UpdateEnviromentDto, userId: string) {
    try {
      ...
      if (newEnviroment) {
        return { statusCode: 201, message: 'Enviroment Created Successfully',enviroment:newEnviroment };
      } else {
        throw new InternalServerErrorException('Unexpected Error Occured');
      }
    } catch (error) {
